function [ind2,phi2] = TheoPhi_and_Espectrogram(v,sentido,qtdeCarros,x1,Fs)
...
for i=1:qtdeCarros     
    ...
    for t2=transicao(i):0.01:transicao(i+1)
        ...
        d2=sqrt((l(ind)+d)^2+(1.26)^2+(4.3)^2);
        delta(ind)=d2-d1;
        if sentido == '0° -> 180°'
           delta(ind) = -delta(ind); 
        end
