function phi = doa_gcc2003TwoSources(alg,x1,x2,dx,N,Fs,v,t90)
...
if alg == 1                                  % SCOT-GCC algorithm
 ...
 %   Cxy = OV*real(ifft(Sxy./(sqrt(Sx.*Sy)+1e-7),Nfo));  % generalized cross-correlation
  ...
  for n = 1:M:Nx-N+1
    ...
    %[Cxymax,imax] = max(Cxy);
    [peaks] = findpeaks(Cxy);
    [pmax1,ipmax1] = max(peaks);
    peaks(ipmax1) = min(peaks);
    pmax2 = max(peaks);
    for k = 1:length(Cxy)
        if Cxy(k) == pmax1
            imax1 = k;
...
delay = (delay(1:m)-Ndo)/(OV*Fs);  
delay2 = (delay2(1:m)-Ndo)/(OV*Fs); % correct delay offset
phi = 180/pi*real(acos(vs/dx*delay)); 
phi2 = 180/pi*real(acos(vs/dx*delay2)); 
...
t2 = M/Fs*[0:length(delay2)-1];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[tTheo,phiTheo] = TheoPhi_and_Espectrogram(v,t90,x1,Fs);
plot(t,phi,t2,phi2,tTheo,phiTheo);

%axis([0 13 0 180],phi2);
