function y = adaptive_beam_modificado(Xin,mics,phi_look,mu,Gw,f,Fs)
...
mic2 = N;
doa_threshold = 0.13;                % CHANGE, if necessary (orignal 0.22)!
Sxy = zeros(1,Nfh);
...
%Cálculo do phi teórico para cada janela
% T=13;
% dT=M*T/(Nx-Nfft+1)
% ind=0;
% ind2=0:dT:T;
...
%     delta(ind)=d2-d1;
%     dt=delta(ind)/343;
%     phi_doa(ind)=acos(delta(ind)/0.2)/pi*180;
% end
% 
% figure
% spectrogram(Xin(:,1),hamming(1024),512,1024,32000,'yaxis')
...
%     phisoma = 0;
%     for i = 1:ind2
%         phisoma = phi2(i) + phisoma;
%     end
%     phi_doa(n) = phisoma/ind2;
    [b,Wc,P] = compute_params(mics,vs,Nfft,Nfmax,theta_d,phi_doa(n),Gw,Fs);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
